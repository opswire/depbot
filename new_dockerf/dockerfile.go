...
func (strategy *Dockerfile) Parse(filePath string, content []byte) ([]*Occurrence, error) {
	...
	for _, node := range result.AST.Children {
		...
		// дочерних аргументных узлов StartLine=0. Используем node.StartLine/EndLine
		// (диапазон FROM-инструкции) для поиска байтового положения образа:
		// при line-continuation образ может быть на строке EndLine, а не StartLine.
		fromLine := node.StartLine
		start, end, found := findValueBytesInRange(content, node.StartLine, node.EndLine, imageStr)
		if !found {
